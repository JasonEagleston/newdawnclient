...
func show_popup(time, text):
	var popup = load("res://scenes/popup_panel.tscn").instantiate()
	self.add_child(popup)
	popup.show_text(time, text)
...
func get_races(buf: PackedByteArray):
	...
	for i in len:
		...
		pos += 1
		available_races[race_name].id = buf.decode_u8(pos)
		pos += 1
		var set_stats = func(buf, pos):
			return {
				"strength" = buf.decode_s64(pos),
				"durability" = buf.decode_s64(pos + 8),
				"force" = buf.decode_s64(pos + 16),
				"resistance" = buf.decode_s64(pos + 24),
				"speed" = buf.decode_s64(pos + 32),
				"recovery" = buf.decode_s64(pos + 40),
				"energy" = buf.decode_s64(pos + 48),
			}
...
func send_creation_stats():
	var stats = $CreationMenu.stat_values
	var data: PackedByteArray = [PacketType.CREATION_STAT_SEND]
	data.append($CreationMenu.available_races[$CreationMenu.selected_race].id)
	var pos = 2
	data.resize(data.size() + 1 * $CreationMenu.stat_names.size())
	for name in $CreationMenu.stat_names:
		data.encode_u8(pos, stats[name])
		pos += 1
	socket.send(data)
